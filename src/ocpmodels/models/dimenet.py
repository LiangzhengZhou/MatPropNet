...
class DimeNetWrap(DimeNet):
    ...
    def forward_features(self, data):
        ...
        if self.training:
            sub_ix = torch.randperm(idx_i.size(0), device=idx_i.device)[
                : self.max_angles_per_image * data.natoms.size(0)
            ]
        ...
        pos_j = pos[idx_j].detach()
        if self.use_pbc:
            pos_ji = pos[idx_j].detach() - pos_i + offsets[idx_ji]
            pos_kj = pos[idx_k].detach() - pos_j + offsets[idx_kj]
        else:
    ...
    @property
    def blocks(self):
        return self.interaction_blocks
