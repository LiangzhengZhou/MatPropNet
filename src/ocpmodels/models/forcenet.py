...
class SafeBatchNorm1d(nn.BatchNorm1d):
    """Bypass BN for degenerate micro-batches during training.

    ForceNet uses BatchNorm1d inside interaction and decoder MLPs. Property-mode
    smoke tests can legitimately run with batch size 1, which makes vanilla
    BatchNorm1d raise at train time. Returning the input unchanged keeps the
    tiny-batch path stable without changing normal multi-sample behavior.
    """
    ...
    def __init__(self, decoder_type, decoder_activation_str, output_dim):
        ...
        if self.decoder_type == "linear":
            ...
            self.decoder = nn.Sequential(
                ...
                SafeBatchNorm1d(self.output_dim),
    ...
    def __init__(
        ...
        if depth_mlp_edge > 0:
            ...
            mlp_edge = [
                ...
                SafeBatchNorm1d(hidden_channels),
            ]
        ...
        if depth_mlp_trans > 0:
            ...
            for i in range(depth_mlp_trans):
                mlp_trans.append(SafeBatchNorm1d(hidden_channels))
