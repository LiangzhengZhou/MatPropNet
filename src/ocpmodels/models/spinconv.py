...
class spinconv(BaseModel):
    ...
    def forward_features(self, data):
        ...
        source_element = atomic_numbers[edge_index[0, :]].long()
        target_element = atomic_numbers[edge_index[1, :]].long()
        x_dist = self.dist_block(edge_distance, source_element, target_element)
        x = self.distfc1(x_dist)
        x = self.act(x)
        x = self.distfc2(x)
        x = self.act(x)
        ...
        for block_index, interaction in enumerate(self.message_blocks):
            ...
            x = x + x_out if block_index > 0 else x_out
