...
class AtomsToGraphs:
    ...
    def _get_neighbors_ase(self, atoms):
        """Fallback neighbor search using ASE for environments where pymatgen's
        C-extension has platform-specific dtype issues."""
        nl = ase.neighborlist.NeighborList(
            [self.radius / 2.0] * len(atoms),
        ...
        for center_idx in range(len(atoms)):
            neigh_idx, neigh_offsets = nl.get_neighbors(center_idx)
            if len(neigh_idx) == 0:
                continue

            neigh_positions = positions[neigh_idx] + np.dot(neigh_offsets, cell)
            neigh_distances = np.linalg.norm(
                neigh_positions - positions[center_idx], axis=1
            )

            order = np.argsort(neigh_distances)[: self.max_neigh]
            center_indices.extend([center_idx] * len(order))
            neighbor_indices.extend(neigh_idx[order].tolist())
            offsets.extend(neigh_offsets[order].tolist())
            distances.extend(neigh_distances[order].tolist())
