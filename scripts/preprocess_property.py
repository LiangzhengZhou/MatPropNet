"""Convert generic materials-property CSV files into LMDB datasets."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--target-columns",
        default=None,
        help="Comma-separated targets. If omitted, infer all non-id/non-cif columns.",
    )
    parser.add_argument(
        "--task-types",
        default=None,
        help="Comma-separated task types aligned with target columns.",
    )
    parser.add_argument(
        "--num-classes",
        default=None,
        help="Comma-separated class counts for classification targets.",
    )
    parser.add_argument(
        "--cif-mode",
        choices=["auto", "inline", "path"],
        default="auto",
    )
...
def build_task_schema(args, fieldnames):
    ...
    task_types = parse_csv_list(args.task_types) or ["regression"] * len(
        target_columns
    )
...
def build_data_object(a2g, row, row_idx, task_schema, args):
    ...
    neighbors = (
        int(data_object.edge_index.shape[1]) if hasattr(data_object, "edge_index") else 0
    )
    ...
    with db.begin(write=True) as txn:
        for row_idx, row in enumerate(tqdm(rows, desc=f"Writing {out_dir}")):
            try:
                ...
                if args.skip_failed:
                    print(f"Skipping row {row_idx} due to error: {exc}", flush=True)
                    continue
                raise
            txn.put(
                f"{written}".encode("ascii"),
...
def main():
    ...
    if args.kfolds and args.kfolds > 1:
        ...
        for fold_idx, test_idx in enumerate(folds):
            ...
            write_manifest(
                os.path.join(fold_dir, "split_manifest.json"),
                split_rows,
                args.id_column,
            )
            for split_name, split_rows_list in split_rows.items():
                write_lmdb_split(
                    split_rows_list,
                    os.path.join(fold_dir, split_name),
                    task_schema,
                    args,
                )
        return
    ...
    if args.split_column:
        ...
        split_rows = random_split(rows, args.split, args.split_seed)
    elif args.out_root:
        split_rows = random_split(rows, (0.8, 0.1, 0.1), args.split_seed)
    else:
        split_rows = None
    ...
    if split_rows is not None:
        ...
        write_manifest(
            os.path.join(args.out_root, "split_manifest.json"),
            split_rows,
            args.id_column,
        )
        summary = {}
        for split_name, split_rows_list in split_rows.items():
            summary[split_name] = write_lmdb_split(
                split_rows_list,
                os.path.join(args.out_root, split_name),
                task_schema,
                args,
            )
    ...
    summary = write_lmdb_split(rows, args.out_path, task_schema, args)
    with open(os.path.join(args.out_path, "summary.json"), "w", encoding="utf-8") as handle:
        json.dump(summary, handle, indent=2)
